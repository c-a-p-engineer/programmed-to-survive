...
export const DATA: {
  ...
  weapons: [
    { id: "mg", name: "Machinegun", type: "bullet", range: 900, dps: 14, cd: 0.1, speed: 1000, desc: "連射。見てて気持ちいい。", pierce: false, explodeOnObstacle: false },
    { id: "cannon", name: "Cannon", type: "bullet", range: 1250, dps: 22, cd: 0.55, speed: 1250, desc: "高威力。遠距離。", pierce: false, explodeOnObstacle: false },
    { id: "missile", name: "Missile", type: "missile", range: 1050, dps: 18, cd: 0.7, speed: 800, radius: 110, desc: "範囲。雑に強い。", pierce: false, explodeOnObstacle: true },
    ...
    { id: "mine", name: "Mine", type: "mine", range: 0, dps: 22, cd: 1.1, speed: 0, radius: 120, armMs: 240, desc: "設置。踏ませる。" }
  ],
  ai: [
    ...
    { id: "hunter", name: "Hunter", keep: 0.65, fleeHp: 0.1, prefer: "mid", desc: "近い敵を確実に。" }
  ]
};

export const WPN = Object.fromEntries(DATA.weapons.map((w) => [w.id, w])) as Record<string, Weapon>;
export const FRAME = Object.fromEntries(DATA.frames.map((f) => [f.id, f])) as Record<string, Frame>;
export const AI = Object.fromEntries(DATA.ai.map((a) => [a.id, a])) as Record<string, AiProfile>;
